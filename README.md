...
- прямые безопасные действия для пустого QR и фото: выбранный блок остаётся включённым, а пользователь переходит к нужному полю;
- безопасный помощник канала отклика: контакты не включаются автоматически, если телефон пустой или похож на ошибочный;

...
- прямое безопасное действие для пустого QR: блок остаётся включённым, а пользователь переходит к полю ссылки;
- безопасные помощники качества для фото и канала отклика: выбранный пользователем блок не выключается автоматически, а интерфейс переводит к нужному полю;

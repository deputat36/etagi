...
### Инвентаризация портфеля шаблонов
...
- повторяющиеся id и office-сценарии;
- пакеты, которым ещё нужна офисная разметка.
...
```text
working      рабочий основной вариант
test         тестовый или требующий актуализации
deprecated   устаревший смысловой дубль
```

Проверка контролирует:

- существование всех templateId из реестра;
- допустимые статусы;
- понятную причину статуса;
- существование `replacementId` для устаревших вариантов;
- отсутствие ссылок на себя, циклов и цепочек устаревших замен;
- загрузку реестра через `assets/js/templates.js`;
- отображение бейджей `Тест` и `Устарел` в карточках.

Устаревший шаблон не удаляется автоматически: он остаётся для совместимости сохранённых макетов, но карточка показывает рекомендуемую замену.
...
Нельзя удалять устаревший шаблон только потому, что появился более точный вариант. Сначала он помечается `deprecated`, получает понятную причину и конечный рабочий `replacementId`.

Тестовый шаблон не должен подаваться новичку как основной офисный вариант без отдельной проверки результата.

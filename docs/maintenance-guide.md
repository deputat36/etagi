...
- проверяет наличие блока backup рабочего пространства;
- открывает и закрывает мобильный режим исполнителя;
...
Для backup и восстановления:

```text
docs/workspace-backup-regression-checklist.md
```

Для мобильного выполнения расклейки:

```text
docs/distribution-field-mode-regression-checklist.md
```

Ручной проход обязателен после изменений старта, выбора шаблона, печати, задания, полевого режима, отчётов, CSV, качества, фото-компоновок, backup и навигации мастера.
...
`spnWorkspaceBackup.js` экспортирует все ключи `localStorage` с префиксом `etagi-raskleyka-`.

В backup попадают профиль, автосохранение, последний и именованные макеты, избранное, режимы интерфейса, Wizard Flow, задания, менеджерские проверки и история отчётов. Фото не входят в backup.

Доступны два режима импорта:

```text
merge    объединить с текущими данными
replace  заменить рабочее пространство
```

Перед записью проверяются схема, версия, размер, префикс ключей и строковый формат значений. Перед заменой требуется подтверждение. При ошибке записи модуль возвращает предыдущий снимок рабочего пространства.

Backup не использует сетевые запросы и не вызывает `localStorage.clear()`. Контракт проверяется `validate:workspace-backup`, browser smoke и отдельным регрессионным чек-листом.

## Мобильное выполнение расклейки

`spnDistributionFieldMode.js` дополняет текстовое задание локальным мобильным диалогом.

Исполнитель может:

- перенести локацию из задания;
- вести до 30 точек;
- отмечать выполненные адреса и подъезды;
- установить статус;
- зафиксировать план/факт листов;
- отметить фотоотчёт без хранения фотографий;
- записать проблемы;
- скопировать итог;
- передать факт и заметки в отчёт после расклейки.

Полевое состояние хранится под ключом `etagi-raskleyka-field-task-v1`, входит в полный backup и не отправляется на сервер. Завершение без фактического количества листов не допускается. Контракт проверяется `validate:distribution-field-mode`, browser smoke и отдельным чек-листом.
...
- безопасный backup и откат при ошибке импорта;
- порядок задания, отчёта и мобильного полевого режима.
...
Нельзя импортировать ключи без префикса проекта, заменять данные без подтверждения или оставлять частично записанное состояние после ошибки.

### Полевой режим

Нельзя хранить фотографии или base64, завершать задание без фактических листов или затирать существующие заметки отчёта.

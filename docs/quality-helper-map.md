...
`assets/js/app.js` импортирует `requestQualityListUpdate()` и вызывает его сразу после того, как `runQuality()` сформировал HTML списка и привязал штатные кнопки исправлений. Поэтому автоматическая и ручная проверка имеют одинаковый явный сигнал завершения. Observer остаётся страховкой для системных замечаний и внешних helper-модулей, которые добавляют или подавляют `.qitem` после основного рендера.
...
- `tools/validate-quality-helper-imports.mjs` — проверяет явный сигнал `app.js`, порядок загрузки, общий канал, side-effect imports и отсутствие удалённых временных helper-файлов;
...
- `tools/quality-list-updates-smoke.html` — проверяет один observer, порядок восьми подписчиков, явный рендер-сигнал, подавление, фильтры, приоритет и защиту печати.
...
Новый основной рендер списка замечаний обязан после заполнения `#qualityList` вызвать `requestQualityListUpdate()`. Нельзя заменять этот сигнал перехватом кнопки, постоянным таймером или ещё одним observer.

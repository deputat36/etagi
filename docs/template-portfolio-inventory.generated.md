...
Сформировано: 2026-08-15T21:37:14.574Z
...
| custom_service_consultation — Пустой: консультация | templates_custom.json | Контролируемая заготовка не является готовой рекламой: перед печатью все смысловые поля заменяются текстом одной услуги и проходят менеджерскую проверку. | — |
| buyer_maternity_capital — Квартира с маткапиталом | templates_extra.json | Рекламный текст утверждён без изменений; перед массовой печатью требуется актуальная проверка правил СФР и, при ипотеке, требований банка. | — |
...
| newbuild_family_mortgage — Новостройка и семейная ипотека | templates_extra.json | Утверждённая редакция не содержит ставки, платежа, взноса или гарантии одобрения; перед использованием проверяются программа, клиент, объект и банк, предупреждение сохраняется полностью. | — |
| seller_empty_flat — Пустует квартира? | templates_extra.json | Утверждённая редакция удаляет ложное «Куплю» и готового покупателя; шаблон предлагает собственнику оценить варианты продажи, аренды или подготовки документов. | — |
...
| trust_service_documents_check — Доверие: проверка документов | templates_trust.json | Утверждённая редакция разделяет роли: СПН собирает исходные данные и передаёт документы профильному специалисту, итоговая правовая оценка не обещается. | — |
...
| buyer_mortgage — Квартира в ипотеку | templates.json | Утверждённая редакция исключает конкретные ставки, платежи и гарантии; шаблон остаётся test и используется только с актуальной проверкой ипотечных условий и полным предупреждением. | — |
...
| newbuild_budget — Новостройка под бюджет | templates.json | Утверждённая редакция убирает «выгоднее», бесплатность и обещание точного платежа; подбор опирается на актуальный бюджет, срок сдачи и предварительный расчёт. | — |
| newbuild_mortgage — Новостройка и ипотека | templates.json | Утверждённая редакция не обещает ставку, платёж, взнос, банки или бесплатность; перед использованием проверяются программа и наличие квартиры, предупреждение сохраняется полностью. | — |
| newbuild_no_commission — Новостройки с понятными условиями | templates.json | Утверждённая редакция убирает абсолютное обещание «без комиссии»; стоимость и порядок услуг объясняются заранее и проверяются по конкретному проекту, договору и прейскуранту. | — |
...
| service_complex_sale — Сложная продажа | templates.json | Рекламный текст сохранён, но использование допускается только после первичного разбора и с обязательной передачей юридически чувствительных вопросов профильному специалисту. | — |
| service_micro_4 — Микро: консультация | templates.json | Утверждённая массовая редакция ограничена продажей или покупкой; ипотечные и юридические темы вынесены в отдельные шаблоны с нужными условиями. | — |
| service_mortgage — Помощь с ипотекой | templates.json | Утверждённая редакция ограничена первичной ипотечной консультацией и предварительным расчётом; заявка и решение банка не обещаются, предупреждение сохраняется полностью. | — |

...
for(const token of ['npm run validate','node tools/validate-performance-budget.mjs','npm run smoke:browser','npm run smoke:ui-actions','npm run performance:budget']){
  if(!workflow.includes(token)) errors.push(`validate.yml: отсутствует ${token}`);
}

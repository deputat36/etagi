...
if (actionsSource && qualitySource) {
  ...
  for (const [title, count] of titleCounts) if (count > 1) errors.push(`assets/js/qualityExtraActions.js: дублируется заголовок — ${title}`);
  for (const item of actions) {
    if (!item.title || !item.action || !item.label) { errors.push(`assets/js/qualityExtraActions.js: неполное описание быстрого исправления — ${JSON.stringify(item)}`); continue; }
...
const requiredActionSnippets = [
  "document.addEventListener('DOMContentLoaded', init)", "list.addEventListener('click', handleBuiltInFixClick, true)", "list.addEventListener('click', handleClick)",
  'new MutationObserver(enhanceQualityList)', "item.querySelector('[data-extra-quality-fix]')", 'type="button"', 'class="quality-extra-fix-btn"',
  'aria-label="${escapeHtml(ariaLabel)}"', 'escapeHtml(config.label)', 'function setInputValue(input, value)',
  "input.dispatchEvent(new Event('input', { bubbles: true }))", "input.dispatchEvent(new Event('change', { bubbles: true }))",
  'function setCheckboxValue(id, checked)', "checkbox.dispatchEvent(new Event('change', { bubbles: true }))", 'function getPrintCount() {', 'function shorten(text, max) {'
];
...
const requiredDirectResponseActionSnippets = [
  "import { cleanPhoneValue, getPhoneInfo } from './phone.js';", "{ title: 'Нет канала отклика', action: 'responseChannel', label: 'Настроить отклик' }",
  "if (action === 'responseChannel') setResponseChannel();", 'function setResponseChannel()', 'getPhoneInfo(getPhoneValue())', "enableCheckbox('showContact');", 'Контакты включены'
];
...
const requiredDirectPhotoActionSnippets = [
  ...
  'function focusPhotoField(inputId, statusText)', "const input = document.getElementById(inputId)", 'Фото оставлено включённым'
];
...
const requiredDirectQrActionSnippets = [
  ...
  "const input = document.getElementById('qrLink')", 'input.select?.()', 'QR оставлен включённым'
];
...
const forbiddenActionSnippets = [
  "document.getElementById('qualityBtn')?.click()", 'function rerunQuality()', 'setInputValue(input, shorten(input.value, 70))', "setInputValue(input, '')",
  "setLayoutExtra('brandName', DEFAULT_BRAND_NAME)", "setLayoutExtra('brandSideText', DEFAULT_BRAND_SIDE)", "{ title: 'QR включён, но ссылки нет', action: 'disableQr'",
  'function disableQr()', 'function hasPhoneValue()', "button.dataset.fix === 'showContact'", 'function hasLikelyPhoneValue()'
];
...
const forbiddenQualitySnippets = [
  "title:'Нет канала отклика', text:'В макете нет контактов, отрывных телефонов и QR. Для расклейки это почти всегда ошибка.', action:'showContact'", "action:'noPhoto'", "action:'onePhoto'"
];
...
const requiredBuiltInMarkup = ['class="quality-fix-btn"', 'aria-label="${esc(ariaLabel)}"', 'data-fix="${i.action}"', 'esc(label)'];
...
const requiredScripts = [
  'assets/js/app.js','assets/js/qualityLevelLabels.js','assets/js/qualityIssueSummary.js','assets/js/qualityPriorityHint.js',
  'assets/js/qualityPrintGuardHint.js','assets/js/qualityIssueFilters.js','assets/js/qualityExtraActions.js'
];
...
function countItems(items) { const counts = new Map(); for (const item of items) counts.set(item, (counts.get(item) || 0) + 1); return counts; }
function escapeRegex(value) { return String(value).replace(/[.*+?^${}()|[\]\\]/g, '\\$&'); }
function readRequired(relativePath) { const filePath = path.join(rootDir, relativePath); return fs.existsSync(filePath) ? fs.readFileSync(filePath, 'utf8') : ''; }

...
const expectedIds = new Set([
  ...
  'buyer_low_budget',
  'object_ready_move_in',
  'object_no_repair_sale',
...
for(const [id, rule] of Object.entries(overrides.templates || {})){
  ...
  if(Number.isInteger(office.recommendedPrintCount) === false || office.recommendedPrintCount < 1 || office.recommendedPrintCount > 12) {
    errors.push(`${label}: office.recommendedPrintCount должен быть целым числом от 1 до 12`);
  }
  if(typeof office.managerNote !== 'string' || office.managerNote.trim().length < 20) {
    errors.push(`${label}: office.managerNote должен объяснять проверку минимум 20 символами`);
  }
...
requireSnippets('assets/js/templates.js', loaderSource, [
  ...
  'normalizeOfficeOverride',
  'emptyOfficeOverrides',
  'office.scenario',
  'office.managerNote',
  '...override.tags'
]);
...
function readObject(filePath, fallback){
  ...
  try{
    ...
    if(!parsed || typeof parsed !== 'object' || Array.isArray(parsed)){
      errors.push(`${path.basename(filePath)}: корень должен быть объектом`);
      return fallback;
    }
    return parsed;
  } catch(error){
    errors.push(`${path.basename(filePath)}: JSON не читается — ${error.message}`);
    return fallback;
  }
}
